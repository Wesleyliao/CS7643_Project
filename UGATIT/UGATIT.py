...
class UGATIT(object):
    ...
    def train(self):
        ...
        if self.resume:
            ...
            if not len(model_list) == 0:
                ...
                self.load(
                    os.path.join(self.result_dir, self.dataset, 'model'), start_iter
                )
                print(" [*] Load SUCCESS")
        ...
        for step in range(start_iter, self.iteration + 1):
            ...
            if step % self.print_freq == 0:
                        ...
                        (
                            ...
                            np.concatenate(
                                (
                                    ...
                                    cam(
                                        tensor2numpy(fake_A2A_heatmap[0]), self.img_size
                                    ),
                                    RGB2BGR(tensor2numpy(denorm(fake_A2A[0]))),
                                    cam(
                                        tensor2numpy(fake_A2B_heatmap[0]), self.img_size
                                    ),
                                    RGB2BGR(tensor2numpy(denorm(fake_A2B[0]))),
                                    cam(
                                        tensor2numpy(fake_A2B2A_heatmap[0]),
                                        self.img_size,
                                    ),
                        ...
                        (
                            ...
                            np.concatenate(
                                (
                                    ...
                                    cam(
                                        tensor2numpy(fake_B2B_heatmap[0]), self.img_size
                                    ),
                                    RGB2BGR(tensor2numpy(denorm(fake_B2B[0]))),
                                    cam(
                                        tensor2numpy(fake_B2A_heatmap[0]), self.img_size
                                    ),
                                    RGB2BGR(tensor2numpy(denorm(fake_B2A[0]))),
                                    cam(
                                        tensor2numpy(fake_B2A2B_heatmap[0]),
                                        self.img_size,
                                    ),
                ...
                cv2.imwrite(
                    os.path.join(
                        self.result_dir, self.dataset, 'img', 'A2B_%07d.png' % step
                    ),
                    A2B * 255.0,
                )
                cv2.imwrite(
                    os.path.join(
                        self.result_dir, self.dataset, 'img', 'B2A_%07d.png' % step
                    ),
                    B2A * 255.0,
                )
                self.genA2B.train(), self.genB2A.train(), self.disGA.train(), self.disGB.train(), self.disLA.train(), self.disLB.train()
            ...
            if step % 1000 == 0:
                ...
                torch.save(
                    params,
                    os.path.join(self.result_dir, self.dataset + '_params_latest.pt'),
                )
    ...
    def test(self):
        ...
        for n, (real_A, _) in enumerate(self.testA_loader):
            ...
            cv2.imwrite(
                os.path.join(
                    self.result_dir, self.dataset, 'test', 'A2B_%d.png' % (n + 1)
                ),
                A2B * 255.0,
            )
        ...
        for n, (real_B, _) in enumerate(self.testB_loader):
            ...
            cv2.imwrite(
                os.path.join(
                    self.result_dir, self.dataset, 'test', 'B2A_%d.png' % (n + 1)
                ),
                B2A * 255.0,
            )
